...
export default function App() {
  ...
  return (
    <div className="site-shell">
      ...
      <footer className="footer"><div className="footer-brand"><img className="footer-logo" src="/brand-assets/dastarkhan-logo.png" alt="Dastarkhan Restaurant and Banquet Hall" /></div><p>Local favourites, family dining, online ordering, and modern banquet facilities for Goala Bazar.</p><div className="footer-links"><a href={ORDER_URL} target="_blank" rel="noreferrer"><ShoppingBag size={16} /> Order online</a><a href={FACEBOOK_URL} target="_blank" rel="noreferrer"><Facebook size={16} /> Facebook</a><a href={`mailto:Dastarkhanbanquethall@gmail.com`}>Email the team</a></div></footer>

      <div className={showCart || showInquiry ? 'mobile-cta modal-open' : 'mobile-cta'}><button onClick={() => goTo('order')}><ShoppingBag size={16} /> Order food</button><a href={`tel:${PHONE}`}><Phone size={16} /> Call</a><button onClick={() => setShowInquiry(true)}><CalendarDays size={16} /> Event quote</button></div>
      {cartCount > 0 && !showCart && !showInquiry && <button className="cart-float" onClick={() => setShowCart(true)}><ShoppingBag size={18} /><span>{cartCount} item{cartCount > 1 ? 's' : ''}</span><strong>৳{cartTotal}</strong><ArrowRight size={16} /></button>}
